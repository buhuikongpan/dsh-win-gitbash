...
export function apply(ctx, config = {}) {
  ...
  // NOTE: we deliberately do NOT touch `ctx.sandbox` here. The sandbox provider
  // (e.g. dsh-sandbox-local) may register after this plugin's apply() runs,
  // because cordis loads plugin entries in parallel (Promise.allSettled) and the
  // sandbox backend loads a native addon, so it can be slower than the policy
  // service. Demanding ctx.sandbox at apply time caused a startup race that
  // threw "ctx.sandbox is missing" on `dsh web` boot. We read it lazily, inside
  // execute(), by which time every plugin has settled.
  if (confining && sandboxPolicyService === undefined) throw new Error('tool-gitbash: the mounted sandbox policy confines but ctx.sandboxPolicy is missing')
  ...
  async function execute(args, exec) {
    ...
    if (policy !== undefined && policy.mode !== 'danger-full-access') {
      // Sandbox provider is resolved lazily here (see apply() note): by tool-call
      // time the dsh-sandbox-local backend has fully registered, so there is no
      // startup race. Fail closed only if it is genuinely absent for a confined
      // composition.
      const sandboxProvider = ctx.get('sandbox')
      if (sandboxProvider === undefined) throw new Error('tool-gitbash: sandboxing is expected for this composition but ctx.sandbox is missing')
