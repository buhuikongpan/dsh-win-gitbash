...
function resolveGitBash() {
  ...
  throw new Error(`tool-gitbash: Git Bash not found — checked ${GIT_BASH_CANDIDATES.join(', ')}`)
}
...
/** Resolve the session cwd first (like the pwsh tool); the policy root is the fallback. */
...
function gitBashDescription(escalationModes) {
...
export function apply(ctx, config = {}) {
  ...
  ctx.tools.register(defineTool({
    name: 'gitbash',
    description: gitBashDescription(escalationModes),
    parameters: {
